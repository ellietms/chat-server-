...
const Form = ({setNewData,SeeLatestMsg}) => {
  ...
  await fetch("https://cyf-ellietms-chat-server.herokuapp.com/messages/newMessage",{
  method: 'POST', 
  headers: {
  ...
  async function SeeLatestMsg(event){
    event.preventDefault();
    await fetch("https://cyf-ellietms-chat-server.herokuapp.com/messages/latest",{
    method: 'GET', 
    headers: {
        'Content-Type': 'application/json'
      },
  ...
  return (
    <div className="container form">
      <form className="mx-auto my-2" action="/messages/newMessage" method="post"
      onSubmit={addNewInfo}>
